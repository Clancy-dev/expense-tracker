...
type PopUpMenuTypes = {
  expense: WeeklyExpenseTypes
  onClose: () => void
}

export default function PopUpMenuWeekly({ expense, onClose }: PopUpMenuTypes) {

   // Getting the monthly daily price
    const monthlyWeeklyPrice = expense.price*4.4;
    ...
    <div className='fixed inset-0 bg-black/40 flex items-end justify-center z-50'>
      ...
      <div className='w-full h-[50vh] bg-white rounded-t-2xl p-6'>
        ...
        <div className='mt-6 space-y-4'>
          <p>
            <span className='font-semibold'>Weekly Expense:</span>{' '}
            UGX {expense.price.toLocaleString()} 
          </p>
           <p>
            <span className='font-semibold'>Monthly Expense:</span>{' '}
            UGX {monthlyWeeklyPrice.toLocaleString()} 
          </p>
